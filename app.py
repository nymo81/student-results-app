...
if uploaded_file:
    try:
        ...
        # ADDED: This shows you the words (Grades) in the app to confirm they work
        if st.checkbox("Show Grade Preview"):
            preview_df = df[['ت', 'اسم الطالب']].copy()
            # Sample check on one subject
            preview_df['تقدير الرياضيات'] = df['الرياضيات'].apply(get_grade)
            st.dataframe(preview_df.head(10))

        if st.button("🚀 Generate PDF & Download"):
            with st.spinner("Processing 360 slips..."):
                try:
                    ...
                    if os.path.exists("Amiri-Regular.ttf"):
                        ...
                        st.error("❌ Font 'Amiri-Regular.ttf' is missing!")
                        st.stop()
                    ...
                    # FIXED: Convert bytearray to bytes for Streamlit
                    pdf_bytes = bytes(pdf.output())
                    
                    st.download_button(
                        label="⬇️ Click Here to Save PDF",
                        data=pdf_bytes,
                        file_name="Final_Result_Slips.pdf",
